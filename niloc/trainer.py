...
def arg_conversion(cfg: DictConfig) -> None:
    ...
    net_config = {"window_size": data_window_config["window_size"]}
...
def assert_and_print(cfg: DictConfig) -> None:
    ...
    logging.info(f"Window Size: {cfg.data_window_cfg.window_size}")
    logging.info(f"Time: {cfg.data.window.window_time}")
...
def configure_output(cfg: DictConfig) -> Tuple[str, pl.loggers.TensorBoardLogger]:
    ...
    tb_logger = pl.loggers.TensorBoardLogger(
        save_dir=root_dir, name="logs", version=version_str
    )
    logging.info(f"Training output will be written to {filepath}")

    if cfg.train_cfg.resume_from_checkpoint is not None and osp.exists(
        cfg.train_cfg.resume_from_checkpoint
    ):
        # use specified checkpoint
        logging.info(f"Resuming from checkpoint {cfg.train_cfg.resume_from_checkpoint}")
    elif (
        checkpoint_dir and osp.exists(checkpoint_dir) and cfg.train_cfg.retry_from_last
    ):
        ...
        if "last.ckpt" in checkpoints:
            ...
            for r in checkpoints:
                if r.endswith(".ckpt") and "epoch=" in r:
                    try:
                        epoch_num = int(
                            r.split("epoch=")[1].split(".ckpt")[0].split("-")[0]
                        )
                        epochs.append(epoch_num)
                        epoch_checkpoints.append(r)
                    except (ValueError, IndexError):
                        # Skip checkpoints that don't match the expected pattern
                        continue
            if epochs:
                epochs = np.asarray(epochs)
                ckpt_name = epoch_checkpoints[np.argmax(epochs)]
                with open_dict(cfg):
                    cfg.train_cfg.resume_from_checkpoint = osp.join(
...
def get_model(cfg: DictConfig) -> pl.LightningModule:
    with open_dict(cfg):
        print(
            f"Overriding output_dim with grid size {cfg.network.output_dim}->{cfg.grid.elements}"
        )
...
def launch_train(cfg: DictConfig) -> None:
    ...
    if cfg.train_cfg.get("load_weights_only", False):
        ...
        with open_dict(cfg):
            ...
            cfg.train_cfg.restore_tr_ratio = cfg.train_cfg.get(
                "restore_tr_ratio", False
            )  # disable by default

        with tempfile.NamedTemporaryFile(suffix=".yaml") as fp:
            OmegaConf.save(config=cfg, f=fp.name)
            network = model_type.load_from_checkpoint(
                cfg.train_cfg.load_weights_only, hparams_file=fp.name
            )
    else:
        network = model_type(cfg)
    logging.info(
        f"Network {cfg.task} & {cfg.arch.name} loaded: Model type {type(network).__name__}"
    )
    ...
    validation_checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath=filepath,
        monitor="val_loss",
    ...
    # Determine accelerator and devices for modern PyTorch Lightning
    accelerator = cfg.train_cfg.get("accelerator", "auto")
    devices = cfg.train_cfg.get("devices", "auto")

    # Override for Apple Silicon MPS if available
    if accelerator == "auto" or accelerator == "ddp":
        if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            accelerator = "mps"
            devices = 1
            logging.info("Using Apple Silicon MPS accelerator")
        elif torch.cuda.is_available():
            accelerator = "gpu"
            devices = cfg.train_cfg.get("gpus", 1)
        else:
            accelerator = "cpu"
            devices = 1

    # Handle legacy gpus parameter
    if "gpus" in cfg.train_cfg and accelerator != "mps":
        devices = cfg.train_cfg.gpus

    # Get checkpoint path (new API uses ckpt_path instead of resume_from_checkpoint)
    ckpt_path = cfg.train_cfg.get("resume_from_checkpoint", None)
    ...
    trainer.fit(network, datamodule=datamodule, ckpt_path=ckpt_path)
    logging.info("Done")
...
def run(cfg: DictConfig) -> None:
    ...
    # Only set CUDA_LAUNCH_BLOCKING if CUDA is available
    if torch.cuda.is_available():
